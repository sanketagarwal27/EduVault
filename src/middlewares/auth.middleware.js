...
export const verifyJwtFaculty = asyncHandler(async(req,res,next) => {
    try {
        const token = req.cookies?.accessToken || req.header("Authorization")?.replace("Bearer ","")
...
export const verifyJwtInstitution = asyncHandler(async(req,res,next) => {
    try {
        const token = req.cookies?.accessToken || req.header("Authorization")?.replace("Bearer ","")
        ...
        const user = await Institution.findById(decodedToken?._id).select("-password -refreshToken")
